...
def update_limit_info(limit_id):
    ...
    for article_in_use in articles_in_use:
        ...
        limit_data[source_num]['articles'][num]['in_use'] = (
            article_in_use['in_use']
        )

    for article_used in articles_used:
        source_num = article_used['source_num']
        num = article_used['num']
        limit_data[source_num]['articles'][num]['used'] = (
            article_used['used_with_delta']
        )
...
def _get_limit_sources(limit):
    ...
    in_use = _get_in_use(qs).values('in_use', num=limit.source_num_field)
    used = _get_used(qs).values('used_with_delta', num=limit.source_num_field)
...
def _get_limit_moneys(limit):
    ...
    total = qs.annotate(
        ...
        'subsidy_code', 'source_num', 'article_num')
...
def _get_in_use(queryset):
    ...
    return queryset \
        ...
        .annotate(in_use=Coalesce(Sum('startprice__money'),
                                  Value(Decimal('0'))))
