...
class Limit(models.Model):
    # TODO: remove this model and optimize relations.
...
class LimitDateInfo(models.Model):
    ...
    data = JSONField(encoder=DecimalJsonEncoder)
...
class Source(models.Model):
    ...
    def get_money(self):
        ...
        return LimitMoney.objects \
            ...
            .aggregate(m=Coalesce(Sum('money'), models.Value(0)))['m']
...
class LimitMoney(models.Model):
    ...
    name = models.CharField(max_length=DEF_CHAR_L)
    subsidy_code = models.CharField(max_length=SUBSIDY_CODE_LENGTH)
...
class Debt(models.Model):
    article = models.OneToOneField(LimitArticle, on_delete=models.CASCADE,
                                   null=True)
    money = models.DecimalField(max_digits=DEF_DECIMAL_DIGITS, default=0,
                                decimal_places=DEF_DECIMAL_PLACES)
